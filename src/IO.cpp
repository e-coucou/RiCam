...
#include "math.h"
// --------------------------------------------------------------------------------------------------------
// BME 280 humidite et temperature
// -
BME280::BME280(void) { }
...
uint16_t BME280::read16(byte reg)
{
  uint16_t value;
    ...
    Wire.requestFrom((uint8_t)devAddr, (byte)2);
    value = (Wire.read() << 8) | Wire.read();
    Wire.endTransmission();

//by EP    I2Cdev::readWord(devAddr, reg, &value);
  return value;
}
uint8_t BME280::read8(byte reg)
{
  uint8_t value;
    ...
    Wire.write((uint8_t)reg);
    Wire.endTransmission();
    Wire.requestFrom((uint8_t)devAddr, (byte)1);
    value = Wire.read();
    Wire.endTransmission();

  return value;

//    I2Cdev::readByte(devAddr, reg, buffer); // by ep ?? 
//    return buffer[0];
}
